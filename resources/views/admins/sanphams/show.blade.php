...
@section('content')
        ...
        <div id="additional-info" class="tab-content active mt-4 container" style="width: 1000px;">
            <h1 class="mt-5 d-flex justify-content-center">CHI TIẾT SẢN PHẨM</h1>
            <form action="#" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="number" class="form-control" name="luot_xem" value="{{ $sanPham->luot_xem }}">
                </div>
                <div class="mb-3 mt-3">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" name="danh_muc_id">
                        ...
                        @foreach ($category as $categorys)
                            <option
                                value="{{ $categorys->id }}"{{ $sanPham->danh_muc_id === $categorys->id ? 'selected' : '' }}>
                                {{ $categorys->ten_danh_muc }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div class="mb-3">
                    ...
                    <i id="add-row" class="mdi mdi-plus text-muted fs-18 rounded-2 border p-1"
                            ...
                            @foreach ($sanPham->hinhAnhSanPham as $item => $hinhAnh)
                                <tr>
                                    <td class="d-flex align-items-center">
                                        <img id="preview_{{ $item }}"
                                            src="{{ Storage::url($hinhAnh->hinh_anh) }}" alt="Hình ảnh sản phẩm"
                                            style="width:50px;" class="me-3">
                                        <input type="file" class="form-control" id="hinh_anh"
                                            name="list_hinh_anh[{{ $hinhAnh->id }}]"
                                            onchange="previewImage(this, {{ $item }})">
            ...
            @if ($sanPham->binhLuans->isNotEmpty())
                <h2>Bình luận</h2>
                <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach ($binhLuan->binhLuans as $binhLuan)
                            <tr>
                                <td>
                                    @if ($binhLuan->taiKhoan)
                                        {{ $binhLuan->taiKhoan->name }}
                                    @else
                                        Người dùng không xác định
                                    @endif
                                </td>
                                <td>{{ $binhLuan->noi_dung }}</td>
                                <td>{{ \Carbon\Carbon::parse($binhLuan->ngay_dang)->format('d/m/Y') }}</td>
                                <td>{{ $binhLuan->danh_gia }} sao</td>
                                <td>
                                    <form action="{{route('sanpham.destroyComment', $binhLuan->id)}}" method="POST" onsubmit="return confirm('Bạn có muốn xoá bình luận này không?')">
                                        @csrf
                                        @method('DELETE')
